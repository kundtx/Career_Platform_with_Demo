...
try:
    ...
    print("[WARNING] failed to import dmPython module. ignore this message if you don't need dmPython")


__all__ = ["MysqlDB", "DamengDB", "PooledMysqlDB", "PooledDamengDB"]
...
class DamengDB(DBInterface):
    """
    对达梦驱动的普通封装.
    """
    def __init__(self, config:Dict=config.datacenter_config):
        self.config = config

    def __get_connection(self):
        while(True):
            try:
                conn = dmPython.connect(**self.config)
            except Exception as E:
                print('Error connecting to database:{0}'.format(E))
                print('Reconnect in 3 sec...')
                time.sleep(3)
...
class PooledDamengDB(DBInterface):
    """
    对达梦数据库连接池的封装.
    本类所有实例共享一个数据库连接池, 连接池在第一个实例创建时初始化, 故config参数仅在第一个实例创建时生效.
    """
    __pool = None       # 连接池对象，只在PooledDamengDB第一次实例化的时候创建，所有实例共享
    __config = None     # 连接配置
