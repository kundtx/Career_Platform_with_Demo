...
from ner.models.bilstm_crf import BILSTM_Model
...
def crf_train_eval(train_data, test_data, remove_O=False):
    ...
    save_model(crf_model, "ner/ckpts/crf.pkl")
...
def bilstm_train_and_eval(train_data,
                          # dev_data,
                          test_data,
                          word2id, tag2id, weight, crf=True, remove_O=False):
    train_word_lists, train_tag_lists = train_data
    ...
    # 给模型加上 pre_trained_weight
    bilstm_model = BILSTM_Model(vocab_size, out_size, weight, crf=crf)
    ...
    save_model(bilstm_model, os.path.join(os.path.dirname(__file__), "ckpts/"+model_name+".pkl"))
