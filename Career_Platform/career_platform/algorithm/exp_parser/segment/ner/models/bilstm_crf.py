...
class BILSTM_Model(object):
    def __init__(self, vocab_size, out_size, weight, crf=True):
        """功能：对LSTM的模型进行训练与测试
           参数:
            vocab_size:词典大小
            out_size:标注种类
    ...
    def train(self, word_lists, tag_lists,
              # dev_word_lists, dev_tag_lists,   # 参数 dev_word_lists, dev_tag_lists 验证数据
              word2id, tag2id):
        ...
        for e in range(1, self.epoches+1):
            self.step = 0
            losses = 0.
            with tqdm(range(0, len(word_lists), B), desc="Train Epoch {}/{}".format(e, self.epoches)) as inds:
                for ind in inds:
                    ...
                    if self.step % TrainingConfig.print_step == 0:
                        inds.set_postfix_str("Loss:{:.4f}".format(losses / self.print_step))
                        losses = 0.

            # 每一轮测试结束之后 修改crf矩阵
            from parser_check import prohibitions, restrictions
            self.model.check_transmat(prohibitions, restrictions, tag2id)
    ...
    def train_step(self, batch_sents, batch_tags, word2id, tag2id):
        ...
        # forward
        scores = self.model(tensorized_sents, lengths)   # [B, L, out_size]
    ...
    def validate(self, dev_word_lists, dev_tag_lists, word2id, tag2id):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model.eval()   # model 进入测试状态
        with torch.no_grad():   # 停止 autograd 模块的工作
            val_losses = 0.
            val_step = 0
            ...
            if val_loss < self._best_val_loss:
                print("保存模型...")
                self.best_model = deepcopy(self.model)   # 深度拷贝
    ...
    def test(self, word_lists, tag_lists, word2id, tag2id):
        ...
        self.best_model.bilstm.bilstm.flatten_parameters()
        with torch.no_grad():
        ...
        # 将id转化为标注
        pred_tag_lists = []
        id2tag = dict((id_, tag) for tag, id_ in tag2id.items())   # 反转字典
        for i, ids in enumerate(batch_tagids):
            tag_list = []
            if self.crf:
                for j in range(lengths[i] - 1):  # crf解码过程中，end被舍弃
                    tag_list.append(id2tag[ids[j].item()])   # ids[j] 也是个张量
            else:
        ...
        # list[enumerate(indices)] = [(新索引位置, 原索引位置)]
        # 索引为2的元素映射到新的索引是1...
...
class BiLSTM_CRF(nn.Module):
    def __init__(self, vocab_size, emb_size, hidden_size, out_size, weight):
        ...
        self.bilstm = BiLSTM(vocab_size, emb_size, hidden_size, out_size, weight)

        # CRF实际上就是多学习一个转移矩阵 [out_size, out_size] 初始化为均匀分布   感觉可以更改
    ...
    def forward(self, sents_tensor, lengths):
        # [B, L, out_size]
        emission = self.bilstm(sents_tensor, lengths)   # BiLSTM 层的输出
        # emission 拓展为 [B, L, out_size, out_size]   后面两个 out_size 可理解为上一时刻和下一时刻 tag
    ...
    def check_transmat(self, prohibitions, restrictions, tag2id):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        tagset_size = len(tag2id)
        for key in prohibitions.keys():
            if key in tag2id.keys():
                index = tag2id[key]
                for item in prohibitions[key]:
                    if item in tag2id.keys():
                        i_p = tag2id[item]
                        self.transition.data[index][i_p] = - 100.

        for key, item_list in restrictions.items():
            if key in tag2id.keys():
                mask = torch.full((tagset_size,), -100.)
                ...
                for item in item_list:
                    if item in tag2id.keys():
                        i_p = tag2id[item]
                        mask[i_p] = 0
                self.transition.data[index] = self.transition.data[index] + mask
